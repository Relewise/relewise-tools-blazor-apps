...
Before requesting review, run the Release build, regression tests, and AOT publish. Explain any unresolved failures or warnings. Use conventional commit subjects such as `fix: preserve nullable editor values`.

PRs target `main`. Put the Trello task URL at the top of the description when available, followed by the problem, resulting behavior, validation performed, and remaining limitations. Open a draft when the work needs discussion or validation is incomplete.
